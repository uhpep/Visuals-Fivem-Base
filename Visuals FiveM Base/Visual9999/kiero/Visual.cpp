...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.lpszClassName = Visual_Text("Visual#9999");
			windowClass.hIconSm = NULL;

			::RegisterClassEx(&windowClass);

			HWND window = ::CreateWindow(windowClass.lpszClassName, Visual_Text("FiveM Ready Cheat Base"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);

			if (_renderType == RenderType::D3D9)
			{
#if VisualDX_D3D9
				HMODULE libD3D9;
				if ((libD3D9 = ::GetModuleHandle(Visual_Text("d3d9.dll"))) == NULL)
				{
			...
			else if (_renderType == RenderType::D3D10)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D10;
				if ((libDXGI = ::GetModuleHandle(Visual_Text("dxgi.dll"))) == NULL || (libD3D10 = ::GetModuleHandle(KIERO_TEXT("d3d10.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
...
#if VisualDX_D3D11
				HMODULE libD3D11;
				if ((libD3D11 = ::GetModuleHandle(Visual_Text("d3d11.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
			...
			else if (_renderType == RenderType::D3D12)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D12;
				if ((libDXGI = ::GetModuleHandle(Visual_Text("dxgi.dll"))) == NULL || (libD3D12 = ::GetModuleHandle(KIERO_TEXT("d3d12.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
...
#if VisualDX_OPENGL
			HMODULE libOpenGL32;
			if ((libOpenGL32 = ::GetModuleHandle(Visual_Text("opengl32.dll"))) == NULL)
			{
				return Status::ModuleNotFoundError;
			}
...
#if VisualDX_VULKAN
			HMODULE libVulkan;
			if ((libVulkan = GetModuleHandle(Visual_Text("vulcan-1.dll"))) == NULL)
			{
				return Status::ModuleNotFoundError;
			}
		...
		else if (_renderType == RenderType::Auto)
		{
			...
			if (::GetModuleHandle(Visual_Text("d3d9.dll")) != NULL)
			{
				type = RenderType::D3D9;
			}
			else if (::GetModuleHandle(Visual_Text("d3d10.dll")) != NULL)
			{
				type = RenderType::D3D10;
			}
			else if (::GetModuleHandle(Visual_Text("d3d11.dll")) != NULL)
			{
				type = RenderType::D3D11;
			}
			else if (::GetModuleHandle(Visual_Text("d3d12.dll")) != NULL)
			{
				type = RenderType::D3D12;
			}
			else if (::GetModuleHandle(Visual_Text("opengl32.dll")) != NULL)
			{
				type = RenderType::OpenGL;
			}
			else if (::GetModuleHandle(Visual_Text("vulcan-1.dll")) != NULL)
			{
